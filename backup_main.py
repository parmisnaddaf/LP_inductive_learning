...
parser.add_argument('-disjoint_transductive_inductive', dest="disjoint_transductive_inductive", default= False , help="This flag is used if want to have dijoint transductive and inductive sets")
...
if args_kdd.CVAE_architecture == "separate":
        ...
        if single_link:
            ...
            for nn in org_adj[idd].nonzero()[0]:
                if single_link_evidence_wo_neighbours:
                    adj_list_copy = copy.deepcopy(org_adj)
                    adj_list_copy = get_single_link_evidence(adj_list_copy, idd, np.delete(adj_list_copy[idd].nonzero()[0], np.where(adj_list_copy[idd].nonzero()[0] == nn)))
                    std_z_recog , m_z_recog , z_recog , re_adj_recog = run_network(features_kdd, adj_list_copy, inductive_pn , is_prior = False)
                    re_adj_recog_sig = torch.sigmoid(re_adj_recog)
                ...
                pred_single_link.append(re_adj_prior_sig[idd,nn].tolist())
                true_single_link.append(org_adj[idd,nn].tolist())
    ...
    if single_link:
        ...
        index = np.where(np.isin(res[:,0],testId)) #only one node of the 2 ends of an edge needs to be in testId
        target_zero_list = res[index]
        std_z_recog , m_z_recog , z_recog , re_adj_recog = run_network(features_kdd, org_adj, inductive_pn , is_prior = False)
        re_adj_recog_sig = torch.sigmoid(re_adj_recog)
        pred_single_link.extend(re_adj_recog_sig[target_zero_list[:false_count,0],target_zero_list[:false_count,1]].tolist())
        true_single_link.extend(org_adj[target_zero_list[:false_count,0],target_zero_list[:false_count,1]].tolist())
        
        auc_list_single, val_acc_list_single, val_ap_list_single, precision_list_single,recall_list_single, HR_list_single = roc_auc_single(pred_single_link, true_single_link)
...
if single_link:
    ...
    print("single link")
    print("auc: ",auc_list_single)
    print("acc", val_acc_list_single)
    print("ap: ",val_ap_list_single)
    print("precision", precision_list_single)
    print("recall",recall_list_single)
    print("HR",HR_list_single)
