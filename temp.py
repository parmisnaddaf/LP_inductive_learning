...
if CVAE_architecture == "separate":
    # run recognition separately
    std_z_rec , m_z_rec , z_rec , re_adj_rec = run_network(features_kdd, adj_list):
...
# GET ALL EMBEDDINGS - recognition
...
# Link Prediction Task
print("=====================================")
print("Result on Link Prediction Task - Between TRAIN edges")
...
for idd in testId:
    ...
    for neighbour_id in non_zero_list[0]:
        ...
        CVAE_list.append(CVAE_loss(m_z_recog,m_z_prior, std_z_recog,std_z_prior, re_adj.detach().numpy(), org_adj, idd, neighbour_id).detach().numpy())
...
P_Y_given_E =  np.mean(np.exp(CVAE_list))
